...
from pydrever.data import DikernelInput, GrassWaveRunupLayerSpecification, TopLayerType, DikeSchematization
    ...
    for x_location in x_locations:
        input.add_output_location(
            ...
            top_layer_specification=GrassWaveRunupLayerSpecification(
                top_layer_type=TopLayerType.GrassClosedSod, outer_slope=get_outer_slope(input.dike_schematization)
            ),
        )
...
def perform_grass_wave_runup_calculation_schematic(
    profile_name: str, profile: DikeSchematization, time_series_name: str, time_series: WaterLevelTimeSeries
):
    calculation_name = "profile " + profile_name + " - wave_runup - " + time_series_name
    ...
    if result[0]:
        store_results(result, calculation_name + ".pyst", grass_wave_runup_output_dir)


def perform_grass_wave_runup_calculations_measured(
    profile_name: str, profile: DikeSchematization, time_series_name: str, time_series: MeasuredSurgeSeries
):
    calculation_name = "profile " + profile_name + " - wave_runup - " + time_series_name + " - gemeten"
    print("Calculating: " + calculation_name, end="")
    ...
    calculation_name = "profile " + profile_name + " - wave_runup - " + time_series_name + " - gefilterd"
    print("Calculating: " + calculation_name, end="")
